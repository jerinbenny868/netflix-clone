...
const SearchPage = () => {
  ...
  const handleTabClick = (tab) => {
    setActiveTab(tab);
    setContentType(tab); // Update content type in the store
    setSearchTerm(""); // Reset search term when switching tabs
    setSearchResults([]); // Reset search results when switching tabs
  };
  ...
  console.log("resulrfffs", searchResults);
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        >
          <input
            ...
            className="flex-grow px-4 py-2 rounded bg-gray-800 text-white focus:outline-none focus:ring-2 focus:ring-red-600"
          />
          <button
            type="submit"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {searchResults.map((result) => {
            if (!result.poster_path && !result.profile_path) return null; // Skip if no image available
            return (
                ...
                {activeTab === "person" ? (
                  ...
                  >
                    <img
                      ...
                      alt={result.name}
                      className="max-h-96 rounded x-auto"
                    />
